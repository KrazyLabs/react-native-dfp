...
-(void)loadBanner
{
  if (_adUnitID && (_bannerSize || _dimensions || _adSizes) && _hasSetCustomTargeting) {
    GADAdSize size;
...
- (void)adView:(DFPBannerView *)bannerView willChangeAdSizeTo:(GADAdSize)size 
{
...
{
  if (self.onAdViewDidFailToReceiveAd) {
    self.onAdViewDidFailToReceiveAd(@{ @"error": [error localizedDescription] });
  }
}
